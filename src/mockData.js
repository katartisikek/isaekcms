// Specialties Seed Data with generated IDs
export const SEED_SPECIALTIES = [
  { "id": "spec_1", "sector": "Τομέας Πληροφορικής", "title": "Τεχνικός Δικτύων και Τηλεπικοινωνίων" },
  { "id": "spec_2", "sector": "Τομέας Πληροφορικής", "title": "Τεχνικός Λογισμικού Ηλεκτρονικών Υπολογιστών" },
  { "id": "spec_3", "sector": "Τομέας Πληροφορικής", "title": "Τεχνικός Εφαρμογών Πληροφορικής" },
  { "id": "spec_4", "sector": "Τομέας Πληροφορικής", "title": "Τεχνικός Υπολογιστικών Συστημάτων" },
  { "id": "spec_5", "sector": "Τομέας Ηλεκτρολογίας και Ηλεκτρονικής", "title": "Τεχνικός Οπτικοακουστικών Διατάξεων και Συστημάτων Ασφάλειας" },
  { "id": "spec_6", "sector": "Τομέας Διοίκησης και Οικονομίας", "title": "Στέλεχος Εμπορίας, Διαφήμισης και Προώθησης Προϊόντων (Marketing)" },
  { "id": "spec_7", "sector": "Τομέας Διοίκησης και Οικονομίας", "title": "Στέλεχος Διοίκησης και Οικονομίας" },
  { "id": "spec_8", "sector": "Τομέας Διοίκησης και Οικονομίας", "title": "Στέλεχος Μηχανογραφημένου Λογιστηρίου - Φοροτεχνικού Γραφείου" },
  { "id": "spec_9", "sector": "Τομέας Διοίκησης και Οικονομίας", "title": "Στέλεχος Δημοσίων Σχέσεων και Επικοινωνίας" },
  { "id": "spec_10", "sector": "Τομέας Διοίκησης και Οικονομίας", "title": "Στέλεχος Διεθνούς Εμπορίου" },
  { "id": "spec_11", "sector": "Τομέας Καλλιτεχνικών Σπουδών & Εφαρμοσμένων Τεχνών", "title": "Εσωτερική Αρχιτεκτονική Διακόσμηση και Σχεδιασμός Αντικειμένων" },
  { "id": "spec_12", "sector": "Τομέας Τουριστικών Επιχ/σεων & Επιχ/σεων Φιλοξενίας", "title": "Τεχνικός Μαγειρικής Τέχνης – Αρχιμάγειρας (Chef)" },
  { "id": "spec_13", "sector": "Τομέας Τουριστικών Επιχ/σεων & Επιχ/σεων Φιλοξενίας", "title": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής" },
  { "id": "spec_14", "sector": "Τομέας Τουριστικών Επιχ/σεων & Επιχ/σεων Φιλοξενίας", "title": "Στέλεχος Μονάδων Φιλοξενίας" },
  { "id": "spec_15", "sector": "Τομέας Τουριστικών Επιχ/σεων & Επιχ/σεων Φιλοξενίας", "title": "Στέλεχος Διοίκησης και Οικονομίας στον Τομέα του Τουρισμού" },
  { "id": "spec_16", "sector": "Τομέας Τουριστικών Επιχ/σεων & Επιχ/σεων Φιλοξενίας", "title": "Στέλεχος Υπηρεσιών Αερομεταφοράς" }
];

// ==========================================
// INITIAL STUDENTS - από Excel ISAEK 2026
// ==========================================
export const INITIAL_STUDENTS = [
  // === ΕΣΩΤΕΡΙΚΗ ΑΡΧΙΤΕΚΤΟΝΙΚΗ ΔΙΑΚΟΣΜΗΣΗ - 2ο ΕΤΟΣ ===
  {
    "id": "stud_1",
    "fullName": "Αποστολάκη Αναστασία",
    "phone": "6980272629",
    "email": "apostolakiana18@gmail.com",
    "specialtyId": "spec_11",
    "mathitisAr": "26",
    "year": "2ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Εσωτερική Αρχιτεκτονική Διακόσμηση και Σχεδιασμός Αντικειμένων - 2ο Έτος"
  },
  {
    "id": "stud_2",
    "fullName": "Αρφανάκη Μαρία",
    "phone": "6943744160",
    "email": "mariaarf02@gmail.com",
    "specialtyId": "spec_11",
    "mathitisAr": "27",
    "year": "2ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Εσωτερική Αρχιτεκτονική Διακόσμηση και Σχεδιασμός Αντικειμένων - 2ο Έτος"
  },
  {
    "id": "stud_3",
    "fullName": "Βασιλειάδη Φωτεινή",
    "phone": "6931835451",
    "email": "fotvas98@gmail.com",
    "specialtyId": "spec_11",
    "mathitisAr": "28",
    "year": "2ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Εσωτερική Αρχιτεκτονική Διακόσμηση και Σχεδιασμός Αντικειμένων - 2ο Έτος"
  },
  {
    "id": "stud_4",
    "fullName": "Γεωργιουδάκη Ελένη",
    "phone": "6946443770",
    "email": "el.georgioudaki@gmail.com",
    "specialtyId": "spec_11",
    "mathitisAr": "29",
    "year": "2ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Εσωτερική Αρχιτεκτονική Διακόσμηση και Σχεδιασμός Αντικειμένων - 2ο Έτος"
  },
  {
    "id": "stud_5",
    "fullName": "Ιεραπετριτάκη Ραφαέλα Δασκαλάκη -",
    "phone": "6947489270",
    "email": "raphaelaierapetritake@gmail.com",
    "specialtyId": "spec_11",
    "mathitisAr": "87",
    "year": "Απόφοιτος 2024Β",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Εσωτερική Αρχιτεκτονική Διακόσμηση - Απόφοιτος 2024Β"
  },
  {
    "id": "stud_6",
    "fullName": "Ζαχαράκης Νικόλαος",
    "phone": "6951734324",
    "email": "nikoszaxarakhs96@yahoo.gr",
    "specialtyId": "spec_11",
    "mathitisAr": "30",
    "year": "Απόφοιτος 2024Β",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Εσωτερική Αρχιτεκτονική Διακόσμηση - Απόφοιτος 2024Β"
  },
  {
    "id": "stud_7",
    "fullName": "Φαρσάρη Βασιλική",
    "phone": "6986849556",
    "email": "farsaryvasiliky@gmail.com",
    "specialtyId": "spec_11",
    "mathitisAr": "61",
    "year": "Απόφοιτος 2024Β",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Εσωτερική Αρχιτεκτονική Διακόσμηση - Απόφοιτος 2024Β"
  },
  {
    "id": "stud_8",
    "fullName": "Φραγκιαδάκη Στυλιανή",
    "phone": "6986697472",
    "email": "stellafragkiadaki23@gmail.com",
    "specialtyId": "spec_11",
    "mathitisAr": "35",
    "year": "Απόφοιτος 2024Β",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Εσωτερική Αρχιτεκτονική Διακόσμηση - Απόφοιτος 2024Β"
  },

  // === ΤΕΧΝΙΚΟΣ ΑΡΤΟΠΟΙΙΑΣ & ΖΑΧΑΡΟΠΛΑΣΤΙΚΗΣ - 1ο ΕΤΟΣ ===
  {
    "id": "stud_20",
    "fullName": "Ανδριανάκη Ειρήνη",
    "phone": "6973805890",
    "email": "iriniandrianaki@gmail.com",
    "specialtyId": "spec_13",
    "mathitisAr": "83",
    "year": "1ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής - 1ο Έτος"
  },
  {
    "id": "stud_21",
    "fullName": "Βελόγλου Γεωργία",
    "phone": "6987666484",
    "email": "georgiaveloglou05@gmail.com",
    "specialtyId": "spec_13",
    "mathitisAr": "88",
    "year": "1ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής - 1ο Έτος"
  },
  {
    "id": "stud_22",
    "fullName": "Καλαφάτη Μαρία-Μαρίνα",
    "phone": "6989801222",
    "email": "merinakalafati33@gmail.com",
    "specialtyId": "spec_13",
    "mathitisAr": "52",
    "year": "1ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής - 1ο Έτος"
  },
  {
    "id": "stud_23",
    "fullName": "Μανωλακάκη Κωνσταντίνα",
    "phone": "6949909988",
    "email": "konstantinaman07@gmail.com",
    "specialtyId": "spec_13",
    "mathitisAr": "55",
    "year": "1ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής - 1ο Έτος"
  },
  {
    "id": "stud_24",
    "fullName": "Ντισπυράκη Γαλάτεια",
    "phone": "6976024153",
    "email": "galateiant@gmail.com",
    "specialtyId": "spec_13",
    "mathitisAr": "54",
    "year": "1ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής - 1ο Έτος"
  },
  {
    "id": "stud_25",
    "fullName": "Παρασύρη Ειρήνη",
    "phone": "6986783854",
    "email": "parasirieirini2007@gmail.com",
    "specialtyId": "spec_13",
    "mathitisAr": "53",
    "year": "1ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής - 1ο Έτος"
  },
  {
    "id": "stud_26",
    "fullName": "Τζανιδάκη Μαρία",
    "phone": "6974068149",
    "email": "tzanidama@yahoo.gr",
    "specialtyId": "spec_13",
    "mathitisAr": "56",
    "year": "1ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής - 1ο Έτος"
  },
  {
    "id": "stud_27",
    "fullName": "Χρηστάκη Μαριλένα",
    "phone": "6945766487",
    "email": "christakim2007@gmail.com",
    "specialtyId": "spec_13",
    "mathitisAr": "51",
    "year": "1ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής - 1ο Έτος"
  },
  {
    "id": "stud_28",
    "fullName": "Λιουδάκη Ζαχαρένια",
    "phone": "6940584115",
    "email": "zaxarenialiou@gmail.com",
    "specialtyId": "spec_13",
    "mathitisAr": "-",
    "year": "1ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής - 1ο Έτος"
  },

  // === ΤΕΧΝΙΚΟΣ ΑΡΤΟΠΟΙΙΑΣ & ΖΑΧΑΡΟΠΛΑΣΤΙΚΗΣ - 2ο ΕΤΟΣ ===
  {
    "id": "stud_30",
    "fullName": "Αλεξή Σοφία Αναργύρη",
    "phone": "6974170536",
    "email": "sophiaaalexe@gmail.com",
    "specialtyId": "spec_13",
    "mathitisAr": "77",
    "year": "2ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής - 2ο Έτος"
  },
  {
    "id": "stud_31",
    "fullName": "Γεωργαντωνάκη Στυλιανή-Γεωργία",
    "phone": "6978474740",
    "email": "sgeorgantonaki@gmail.com",
    "specialtyId": "spec_13",
    "mathitisAr": "79",
    "year": "2ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής - 2ο Έτος"
  },
  {
    "id": "stud_32",
    "fullName": "Ελληνικάκης Γιάννης",
    "phone": "6986405541",
    "email": "giannisellinikakis511@gmail.com",
    "specialtyId": "spec_13",
    "mathitisAr": "39",
    "year": "2ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής - 2ο Έτος"
  },
  {
    "id": "stud_33",
    "fullName": "Καρατζής Βασίλειος",
    "phone": "6978161053",
    "email": "vasiliskarantz@gmail.com",
    "specialtyId": "spec_13",
    "mathitisAr": "78",
    "year": "2ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής - 2ο Έτος"
  },
  {
    "id": "stud_34",
    "fullName": "Κουμάκη Μαρία",
    "phone": "6985725551",
    "email": "makoumaki@gmail.com",
    "specialtyId": "spec_13",
    "mathitisAr": "41",
    "year": "2ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής - 2ο Έτος"
  },
  {
    "id": "stud_35",
    "fullName": "Κουναλάκης Σταύρος",
    "phone": "6946868881",
    "email": "stauroskoun@hotmail.com",
    "specialtyId": "spec_13",
    "mathitisAr": "40",
    "year": "2ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής - 2ο Έτος"
  },
  {
    "id": "stud_36",
    "fullName": "Κυπραίος Μιχάλης",
    "phone": "6944098848",
    "email": "michaleskypraios492@gmail.com",
    "specialtyId": "spec_13",
    "mathitisAr": "43",
    "year": "2ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής - 2ο Έτος"
  },
  {
    "id": "stud_37",
    "fullName": "Μαλλιωτάκη Φιλοξένη",
    "phone": "6993928303",
    "email": "malliotxenia@gmail.com",
    "specialtyId": "spec_13",
    "mathitisAr": "44",
    "year": "2ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής - 2ο Έτος"
  },
  {
    "id": "stud_38",
    "fullName": "Σκορδύλης Γεώργιος",
    "phone": "6949917351",
    "email": "giwrgosskordulhs@gmail.com",
    "specialtyId": "spec_13",
    "mathitisAr": "76",
    "year": "2ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής - 2ο Έτος"
  },
  {
    "id": "stud_39",
    "fullName": "Σκούλα Γεωργία",
    "phone": "6943445469",
    "email": "georgia.sk1708@gmail.com",
    "specialtyId": "spec_13",
    "mathitisAr": "80",
    "year": "2ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής - 2ο Έτος"
  },

  // === ΤΕΧΝΙΚΟΣ ΑΡΤΟΠΟΙΙΑΣ & ΖΑΧΑΡΟΠΛΑΣΤΙΚΗΣ - ΑΠΟΦΟΙΤΟΙ 2023Β ===
  {
    "id": "stud_40",
    "fullName": "Γιακουμάκης Χρήστος",
    "phone": "6973800907",
    "email": "annakotridou@yahoo.gr",
    "specialtyId": "spec_13",
    "mathitisAr": "-",
    "year": "Απόφοιτος 2023Β",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής - Απόφοιτος 2023Β"
  },
  {
    "id": "stud_41",
    "fullName": "Μαραγκάκη Ελένη",
    "phone": "6971911553",
    "email": "eleni75maragkaki@gmail.com",
    "specialtyId": "spec_13",
    "mathitisAr": "6",
    "year": "Απόφοιτος 2023Β",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής - Απόφοιτος 2023Β"
  },
  {
    "id": "stud_42",
    "fullName": "Παπαβασιλείου Μαρία",
    "phone": "6988607936",
    "email": "maria_papadmm@yahoo.gr",
    "specialtyId": "spec_13",
    "mathitisAr": "9",
    "year": "Απόφοιτος 2023Β",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής - Απόφοιτος 2023Β"
  },
  {
    "id": "stud_43",
    "fullName": "Παρασύρη Παρασκευή Αγγελική",
    "phone": "6947161430",
    "email": "vikiparasiri@gmail.com",
    "specialtyId": "spec_13",
    "mathitisAr": "17",
    "year": "Απόφοιτος 2023Β",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής - Απόφοιτος 2023Β"
  },
  {
    "id": "stud_44",
    "fullName": "Περάκης Γεώργιος",
    "phone": "6949227372",
    "email": "perakisg@gmail.com",
    "specialtyId": "spec_13",
    "mathitisAr": "18",
    "year": "Απόφοιτος 2023Β",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής - Απόφοιτος 2023Β"
  },
  {
    "id": "stud_45",
    "fullName": "Σεληθωμάς Γεώργιος",
    "phone": "6984586165",
    "email": "selithomasgiorgos@gmail.com",
    "specialtyId": "spec_13",
    "mathitisAr": "19",
    "year": "Απόφοιτος 2023Β",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής - Απόφοιτος 2023Β"
  },
  {
    "id": "stud_46",
    "fullName": "Σπαντιδάκης Εμμανουήλ",
    "phone": "6972517203",
    "email": "manolesspantidakes@gmail.com",
    "specialtyId": "spec_13",
    "mathitisAr": "20",
    "year": "Απόφοιτος 2023Β",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής - Απόφοιτος 2023Β"
  },
  {
    "id": "stud_47",
    "fullName": "Τζωρτζάκη Στυλιανή",
    "phone": "6941447919",
    "email": "tzortzakistella5@gmail.com",
    "specialtyId": "spec_13",
    "mathitisAr": "21",
    "year": "Απόφοιτος 2023Β",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής - Απόφοιτος 2023Β"
  },

  // === ΤΕΧΝΙΚΟΣ ΜΑΓΕΙΡΙΚΗΣ ΤΕΧΝΗΣ - 1ο ΕΤΟΣ ===
  {
    "id": "stud_50",
    "fullName": "Χριστοδουλάκης Γεώργιος",
    "phone": "6988281254",
    "email": "christodoulakesgiorgos57@gmail.com",
    "specialtyId": "spec_12",
    "mathitisAr": "-",
    "year": "1ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Μαγειρικής Τέχνης – Αρχιμάγειρας (Chef) - 1ο Έτος"
  },
  {
    "id": "stud_51",
    "fullName": "Σκουμπάκης Αντώνιος",
    "phone": "6942286531",
    "email": "askoumpakis20@gmail.com",
    "specialtyId": "spec_12",
    "mathitisAr": "-",
    "year": "1ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Μαγειρικής Τέχνης – Αρχιμάγειρας (Chef) - 1ο Έτος"
  },

  // === ΤΕΧΝΙΚΟΣ ΜΑΓΕΙΡΙΚΗΣ ΤΕΧΝΗΣ - ΑΠΟΦΟΙΤΟΙ 2023Β ===
  {
    "id": "stud_60",
    "fullName": "Fecanji Naze",
    "phone": "6906851518",
    "email": "nazefecanji@gmail.com",
    "specialtyId": "spec_12",
    "mathitisAr": "11",
    "year": "Απόφοιτος 2023Β",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Μαγειρικής Τέχνης - Απόφοιτος 2023Β"
  },
  {
    "id": "stud_61",
    "fullName": "Λίτσης Δημήτριος",
    "phone": "6940332546",
    "email": "Demetreslitses@gmail.com",
    "specialtyId": "spec_12",
    "mathitisAr": "12",
    "year": "Απόφοιτος 2023Β",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Μαγειρικής Τέχνης - Απόφοιτος 2023Β"
  },
  {
    "id": "stud_62",
    "fullName": "Μανταφάρα Βασιλική",
    "phone": "6976616585",
    "email": "basomad@gmail.com",
    "specialtyId": "spec_12",
    "mathitisAr": "13",
    "year": "Απόφοιτος 2023Β",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Μαγειρικής Τέχνης - Απόφοιτος 2023Β"
  },
  {
    "id": "stud_63",
    "fullName": "Μαράκης Μιχαήλ",
    "phone": "6970651783",
    "email": "mixalismarakis2005@gmail.com",
    "specialtyId": "spec_12",
    "mathitisAr": "14",
    "year": "Απόφοιτος 2023Β",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Μαγειρικής Τέχνης - Απόφοιτος 2023Β"
  },
  {
    "id": "stud_64",
    "fullName": "Μυλωνάκης Μιχαήλ",
    "phone": "6934025019",
    "email": "mmylonakis2004@gmail.com",
    "specialtyId": "spec_12",
    "mathitisAr": "15",
    "year": "Απόφοιτος 2023Β",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Μαγειρικής Τέχνης - Απόφοιτος 2023Β"
  },
  {
    "id": "stud_65",
    "fullName": "Ταταρής Ελευθέριος",
    "phone": "6973549071",
    "email": "tataris2@yahoo.gr",
    "specialtyId": "spec_12",
    "mathitisAr": "-",
    "year": "Απόφοιτος 2023Β",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Μαγειρικής Τέχνης - Απόφοιτος 2023Β"
  },
  {
    "id": "stud_66",
    "fullName": "Τσαφαντάκης Δημήτριος",
    "phone": "6932736849",
    "email": "tsafantakisd@gmail.com",
    "specialtyId": "spec_12",
    "mathitisAr": "-",
    "year": "Απόφοιτος 2023Β",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Μαγειρικής Τέχνης - Απόφοιτος 2023Β"
  },
  {
    "id": "stud_67",
    "fullName": "Φουναργιωτάκη Μαρίνα",
    "phone": "6975793246",
    "email": "marinafounargiotaki10@gmail.com",
    "specialtyId": "spec_12",
    "mathitisAr": "16",
    "year": "Απόφοιτος 2023Β",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Τεχνικός Μαγειρικής Τέχνης - Απόφοιτος 2023Β"
  },

  // === ΣΤΕΛΕΧΟΣ ΥΠΗΡΕΣΙΩΝ ΑΕΡΟΜΕΤΑΦΟΡΑΣ - 2ο ΕΤΟΣ ===
  {
    "id": "stud_70",
    "fullName": "Μανωλακάκη Μαρία",
    "phone": "6947841222",
    "email": "manolakakimaria@yahoo.gr",
    "specialtyId": "spec_16",
    "mathitisAr": "46",
    "year": "2ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Στέλεχος Υπηρεσιών Αερομεταφοράς - 2ο Έτος"
  },
  {
    "id": "stud_71",
    "fullName": "Μαραγκάκη Μαρία",
    "phone": "6937357077",
    "email": "mariamaragkaki@yahoo.gr",
    "specialtyId": "spec_16",
    "mathitisAr": "47",
    "year": "2ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Στέλεχος Υπηρεσιών Αερομεταφοράς - 2ο Έτος"
  },
  {
    "id": "stud_72",
    "fullName": "Σμυρνάκη Κωνσταντίνα",
    "phone": "6978163340",
    "email": "nantiasmirnaki@gmail.com",
    "specialtyId": "spec_16",
    "mathitisAr": "48",
    "year": "2ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Στέλεχος Υπηρεσιών Αερομεταφοράς - 2ο Έτος"
  },
  {
    "id": "stud_73",
    "fullName": "Σταματάκη Ουρανία",
    "phone": "6986184307",
    "email": "rania.stamatakiiii@gmail.com",
    "specialtyId": "spec_16",
    "mathitisAr": "49",
    "year": "2ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Στέλεχος Υπηρεσιών Αερομεταφοράς - 2ο Έτος"
  },
  {
    "id": "stud_74",
    "fullName": "Σταματάκης Αντώνιος",
    "phone": "6943696191",
    "email": "antonis.stamatakis2@yahoo.com",
    "specialtyId": "spec_16",
    "mathitisAr": "50",
    "year": "2ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Στέλεχος Υπηρεσιών Αερομεταφοράς - 2ο Έτος"
  },

  // === ΣΤΕΛΕΧΟΣ ΔΙΟΙΚΗΣΗΣ ΚΑΙ ΟΙΚΟΝΟΜΙΑΣ ===
  {
    "id": "stud_80",
    "fullName": "Παπαδομανωλάκης Δημήτρης",
    "phone": "6945452188",
    "email": "dpapadomanolakis@uniwa.gr",
    "specialtyId": "spec_7",
    "mathitisAr": "-",
    "year": "1ο Έτος",
    "totalDebt": 0,
    "hasInstallments": false,
    "numberOfInstallments": 1,
    "notes": "Στέλεχος Διοίκησης και Οικονομίας"
  }
];

// ==========================================
// INITIAL TASKS
// ==========================================
export const INITIAL_TASKS = [
  {
    "id": "task_1",
    "title": "Αναμονή βαθμολογίας πληροφορικής",
    "description": "Αναμονή των τελικών βαθμολογιών εξαμήνου από τους εκπαιδευτές πληροφορικής για καταχώρηση στο σύστημα.",
    "status": "Εκκρεμεί",
    "priority": "Κανονική",
    "assignee": "Κατερίνα",
    "dueDate": "2026-07-05"
  },
  {
    "id": "task_2",
    "title": "Έλεγχος δικαιολογητικών τουριστικών",
    "description": "Έλεγχος ταυτοτήτων και απολυτηρίων λυκείου για τις νέες εγγραφές στον τομέα τουρισμού.",
    "status": "Σε Εξέλιξη",
    "priority": "Επείγον",
    "assignee": "Μαίρη",
    "dueDate": "2026-06-30"
  },
  {
    "id": "task_3",
    "title": "Έκδοση βεβαιώσεων σπουδών",
    "description": "Έκδοση και ηλεκτρονική αποστολή βεβαιώσεων φοίτησης για στρατολογική χρήση.",
    "status": "Ολοκληρώθηκε",
    "priority": "Κανονική",
    "assignee": "Κατερίνα",
    "dueDate": "2026-06-20"
  },
  {
    "id": "task_4",
    "title": "Ενημέρωση αρχείου εκπαιδευτών",
    "description": "Ανανέωση στοιχείων εκπαιδευτών για το εξάμηνο 2026Α.",
    "status": "Εκκρεμεί",
    "priority": "Κανονική",
    "assignee": "Γραμματεία",
    "dueDate": "2026-07-15"
  }
];

// ==========================================
// INITIAL CONTACTS - Συνεργάτες & Εκπαιδευτές & Προμηθευτές
// ==========================================
export const INITIAL_CONTACTS = [
  // === ΥΠΗΡΕΣΙΕΣ / ΑΡΧΕΣ ===
  {
    "id": "contact_1",
    "name": "Υπουργείο Παιδείας & Θρησκευμάτων",
    "category": "Υπουργείο / Πήγασος",
    "phone": "2103442000",
    "email": "info@minedu.gov.gr",
    "description": "Κεντρικό πρωτόκολλο, Διεύθυνση Διά Βίου Μάθησης, θέματα Πήγασου"
  },
  {
    "id": "contact_2",
    "name": "Τεχνικό Τμήμα Πήγασου",
    "category": "Υπουργείο / Πήγασος",
    "phone": "2103442018",
    "email": "",
    "description": "Τεχνική υποστήριξη πλατφόρμας ΠΗΓΑΣΟΣ"
  },
  {
    "id": "contact_3",
    "name": "Κοτανίδη - Υπουργείο",
    "category": "Υπουργείο / Πήγασος",
    "phone": "2103442418",
    "email": "ekotanidi@minedu.gov.gr",
    "description": "Αρμόδια για θέματα ΠΗΓΑΣΟΣ - Υπουργείο Παιδείας"
  },

  // === ΕΚΠΑΙΔΕΥΤΕΣ ===
  {
    "id": "contact_10",
    "name": "Λυρατζάκη",
    "category": "Εκπαιδευτής",
    "specialtyId": "spec_11",
    "phone": "",
    "email": "",
    "description": "Εκπαιδευτής Εσωτερικής Αρχιτεκτονικής - Φυσικά Πετρώματα, Γραμμικό Σχέδιο, Τεχνητά Υλικά"
  },
  {
    "id": "contact_11",
    "name": "Χατζηντεβέ",
    "category": "Εκπαιδευτής",
    "specialtyId": "spec_11",
    "phone": "",
    "email": "",
    "description": "Εκπαιδευτής Εσωτερικής Αρχιτεκτονικής - Παλαιότεροι Πολιτισμοί και Τέχνη, Ελεύθερο και Βασικό Σχέδιο"
  },
  {
    "id": "contact_12",
    "name": "Μπίτζου",
    "category": "Εκπαιδευτής",
    "specialtyId": "spec_11",
    "phone": "",
    "email": "",
    "description": "Εκπαιδευτής Εσωτερικής Αρχιτεκτονικής - Πρακτική Εφαρμογή, Σύγχρονη και Ελληνική Τέχνη"
  },
  {
    "id": "contact_13",
    "name": "Τζανάκη",
    "category": "Εκπαιδευτής",
    "specialtyId": "spec_11",
    "phone": "",
    "email": "",
    "description": "Εκπαιδευτής Εσωτερικής Αρχιτεκτονικής - Τεχνητά Υλικά, Κατασκευαστικό Σχέδιο Εσωτερικών Χώρων"
  },
  {
    "id": "contact_14",
    "name": "Σκόρδου",
    "category": "Εκπαιδευτής",
    "specialtyId": "spec_11",
    "phone": "",
    "email": "",
    "description": "Εκπαιδευτής Εσωτερικής Αρχιτεκτονικής - Διακοσμητικές Εφαρμογές, Πρακτική Εφαρμογή σε Χώρους Εργασίας"
  },
  {
    "id": "contact_15",
    "name": "Μπουχαλάκης Μανώλης",
    "category": "Εκπαιδευτής",
    "specialtyId": "spec_16",
    "phone": "",
    "email": "",
    "description": "Εκπαιδευτής Στελέχους Αερομεταφοράς - Business English & Tourism Terminology, Εισαγωγή στην Αεροπορική Ορολογία"
  },
  {
    "id": "contact_16",
    "name": "Σάβα Μικαέλα",
    "category": "Εκπαιδευτής",
    "specialtyId": "spec_16",
    "phone": "",
    "email": "",
    "description": "Εκπαιδευτής Στελέχους Αερομεταφοράς - Γαλλικά, Εισαγωγή στην Αεροπορική Ορολογία στα Γαλλικά"
  },
  {
    "id": "contact_17",
    "name": "Μαρκάκη Τρυπάκη Πελαγία",
    "category": "Εκπαιδευτής",
    "specialtyId": "spec_16",
    "phone": "",
    "email": "",
    "description": "Εκπαιδευτής Στελέχους Αερομεταφοράς - Τουρισμός και Αεροπορική Βιομηχανία, Ταξιδιωτική Γεωγραφία"
  },
  {
    "id": "contact_18",
    "name": "Καλογεράκης Μιχάλης",
    "category": "Εκπαιδευτής",
    "specialtyId": "spec_16",
    "phone": "",
    "email": "",
    "description": "Εκπαιδευτής Στελέχους Αερομεταφοράς - Αεροπορικό Δίκαιο και Διεθνείς Σχέσεις (IATA – UFTAA – ICAO – BSP)"
  },
  {
    "id": "contact_19",
    "name": "Ρεπανάκη Ντέπη",
    "category": "Εκπαιδευτής",
    "specialtyId": "spec_16",
    "phone": "",
    "email": "",
    "description": "Εκπαιδευτής Στελέχους Αερομεταφοράς - Αεροπορικοί Ναύλοι Α + Β Εξάμηνο"
  },
  {
    "id": "contact_20",
    "name": "Ψαθάκης Μιχαήλ",
    "category": "Εκπαιδευτής",
    "specialtyId": "spec_13",
    "phone": "6972636648",
    "email": "",
    "description": "Εκπαιδευτής - Ασφάλεια και Υγιεινή, Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής (ΣΥΜΒΑΣΗ)"
  },
  {
    "id": "contact_21",
    "name": "Φυγετάκη Μαρία",
    "category": "Εκπαιδευτής",
    "specialtyId": "spec_16",
    "phone": "",
    "email": "",
    "description": "Εκπαιδευτής Στελέχους Αερομεταφοράς - Εισαγωγή στα Συστήματα Κρατήσεων & Κατασκευή Αεροπορικού Ναύλου"
  },
  {
    "id": "contact_22",
    "name": "Αναγνωστάκη Νίκη",
    "category": "Εκπαιδευτής",
    "specialtyId": "spec_7",
    "phone": "6983756076",
    "email": "ni.anagnostaki@gmail.com",
    "description": "Εκπαιδευτής Στελέχους Διοίκησης και Οικονομίας - Αστικό Δίκαιο (ΤΙΜΟΛΟΓΙΟ)"
  },
  {
    "id": "contact_23",
    "name": "Ζεάκη Εμμανουέλα",
    "category": "Εκπαιδευτής",
    "specialtyId": "spec_7",
    "phone": "",
    "email": "",
    "description": "Εκπαιδευτής Στελέχους Διοίκησης και Οικονομίας - Εισαγωγή στα Οικονομικά Μαθηματικά, Στατιστική"
  },
  {
    "id": "contact_24",
    "name": "Παπαδομανωλάκης Δημήτρης",
    "category": "Εκπαιδευτής",
    "specialtyId": "spec_7",
    "phone": "6945452188",
    "email": "dpapadomanolakis@uniwa.gr",
    "description": "Εκπαιδευτής Στελέχους Διοίκησης και Οικονομίας - Εισαγωγή στη Διοίκηση Επιχειρήσεων (ΣΥΜΒΑΣΗ)"
  },
  {
    "id": "contact_25",
    "name": "Φυσαράκης Δημήτρης",
    "category": "Εκπαιδευτής",
    "specialtyId": "spec_13",
    "phone": "",
    "email": "",
    "description": "Εκπαιδευτής Αρτοποιίας – Ζαχαροπλαστικής - Ζαχαροπλαστική Τέχνη I, II"
  },
  {
    "id": "contact_26",
    "name": "Απλαδά Σοφία - Χαρά",
    "category": "Εκπαιδευτής",
    "specialtyId": "spec_13",
    "phone": "",
    "email": "",
    "description": "Εκπαιδευτής Αρτοποιίας – Ζαχαροπλαστικής - Στοιχεία Μαγειρικής (Catering) I"
  },
  {
    "id": "contact_27",
    "name": "Νεονάκη Μαρίνα",
    "category": "Εκπαιδευτής",
    "specialtyId": "spec_13",
    "phone": "6972089035",
    "email": "neonakimar@yahoo.gr",
    "description": "Εκπαιδευτής Αρτοποιίας – Ζαχαροπλαστικής - Υγιεινή και Ασφάλεια, Τρόφογνωσία (ΣΥΜΒΑΣΗ)"
  },
  {
    "id": "contact_28",
    "name": "Κουμάκης Κώστας",
    "category": "Εκπαιδευτής",
    "specialtyId": "spec_13",
    "phone": "6974120129",
    "email": "info@zaxaroplasteiokoumakis.gr",
    "description": "Εκπαιδευτής Αρτοποιίας – Ζαχαροπλαστικής - Πρακτική Εφαρμογή, Ζαχαροπλαστική Τέχνη ΙΙΙ (ΤΙΜΟΛΟΓΙΟ)"
  },
  {
    "id": "contact_29",
    "name": "Χανιωτάκης Χαράλαμπος",
    "category": "Εκπαιδευτής",
    "specialtyId": "spec_13",
    "phone": "6977586273",
    "email": "babis.chaniotakis@gmail.com",
    "description": "Εκπαιδευτής Αρτοποιίας – Ζαχαροπλαστικής - Διαχείριση Πρώτων Υλών ΙΙ, Οργάνωση και Λειτουργία Αρτοζαχαροπλαστείου (ΣΥΜΒΑΣΗ)"
  },
  {
    "id": "contact_30",
    "name": "Βογιατζής Διονύσης",
    "category": "Εκπαιδευτής",
    "specialtyId": "spec_13",
    "phone": "6946076210",
    "email": "nioniosvog@hotmail.com",
    "description": "Εκπαιδευτής Αρτοποιίας – Ζαχαροπλαστικής - Αρτοποιητική Τέχνη Ι, ΙΙ (ΤΙΜΟΛΟΓΙΟ)"
  },
  {
    "id": "contact_31",
    "name": "Κυρλάκης Μάριος",
    "category": "Εκπαιδευτής",
    "specialtyId": "spec_12",
    "phone": "",
    "email": "",
    "description": "Εκπαιδευτής Μαγειρικής Τέχνης - Τέχνη Μαγειρικής Ι, Πρακτική Εφαρμογή Α΄"
  },
  {
    "id": "contact_32",
    "name": "Πετρουλάκη Τόνια",
    "category": "Εκπαιδευτής",
    "specialtyId": "spec_12",
    "phone": "",
    "email": "",
    "description": "Εκπαιδευτής Μαγειρικής Τέχνης - Τέχνη Μαγειρικής Ι, Πρακτική Εφαρμογή Α΄"
  },

  // === ΣΥΝΕΡΓΑΤΕΣ ===
  {
    "id": "contact_40",
    "name": "Λεκάκης",
    "category": "Συνεργάτης",
    "phone": "6948003041",
    "email": "",
    "description": "Ξυλουργός"
  },
  {
    "id": "contact_41",
    "name": "Καπετάνιος",
    "category": "Συνεργάτης",
    "phone": "6946796673",
    "email": "",
    "description": "Ηλεκτρολόγος"
  },
  {
    "id": "contact_42",
    "name": "Γρηγορέλης Χάρης",
    "category": "Συνεργάτης",
    "phone": "6932989892",
    "email": "",
    "description": "Απολύμανση"
  },
  {
    "id": "contact_43",
    "name": "Σάββας",
    "category": "Συνεργάτης",
    "phone": "6945462487",
    "email": "",
    "description": "Κηπουρός"
  },
  {
    "id": "contact_44",
    "name": "Σταματάκη Ραφαέλα",
    "category": "Συνεργάτης",
    "phone": "6946352918",
    "email": "",
    "description": "CANDIA - Καθαριότητα"
  },
  {
    "id": "contact_45",
    "name": "Σταματάκης Γεώργιος",
    "category": "Συνεργάτης",
    "phone": "6984498833",
    "email": "",
    "description": "CANDIA - Καθαριότητα"
  },
  {
    "id": "contact_46",
    "name": "Αβραάμ Νίκος",
    "category": "Συνεργάτης",
    "phone": "6948004343",
    "email": "",
    "description": "Πλακατζής"
  },
  {
    "id": "contact_47",
    "name": "Αβραάμ Γρηγόριος",
    "category": "Συνεργάτης",
    "phone": "2810314476",
    "email": "",
    "description": "Ασφάλεια κτιρίου"
  },
  {
    "id": "contact_48",
    "name": "Καρίτης",
    "category": "Συνεργάτης",
    "phone": "6947004574",
    "email": "",
    "description": "Συντήρηση"
  },
  {
    "id": "contact_49",
    "name": "Καραπιπεράκης",
    "category": "Συνεργάτης",
    "phone": "6972310425",
    "email": "",
    "description": "Δίκτυα - IT Τεχνικών Σχολών"
  },
  {
    "id": "contact_50",
    "name": "Αμπαρτζάκη Στέλλα",
    "category": "Συνεργάτης",
    "phone": "2810240140",
    "email": "stellaampartzaki@hotmail.com",
    "description": "Εξωτερικό Λογιστήριο"
  },
  {
    "id": "contact_51",
    "name": "Αστρινάκης Γιάννης",
    "category": "Συνεργάτης",
    "phone": "6953003616",
    "email": "",
    "description": "Γυψοσανίδες"
  },
  {
    "id": "contact_52",
    "name": "Αρμάντο Μπαϊράμη - LUXUS Cleaning",
    "category": "Συνεργάτης",
    "phone": "6992327304",
    "email": "luxuscleaning13@gmail.com",
    "description": "Καθαριότητα"
  },
  {
    "id": "contact_53",
    "name": "Ζαφείρης",
    "category": "Συνεργάτης",
    "phone": "6974825469",
    "email": "",
    "description": "Υδραυλικός - Επισκευές βρυσών κλπ"
  },
  {
    "id": "contact_54",
    "name": "Καράτζης Παντελής KMG",
    "category": "Συνεργάτης",
    "phone": "6985016410",
    "email": "",
    "description": "Υπεύθυνος για τα δίκτυα (π.χ. Internet)"
  },
  {
    "id": "contact_55",
    "name": "Αντώνης Καλογεράκης",
    "category": "Συνεργάτης",
    "phone": "6989416655",
    "email": "socialbuildersgr@gmail.com",
    "description": "Βοήθεια στη διαχείριση Social Media"
  },
  {
    "id": "contact_56",
    "name": "Μελεσσανάκη Έλενα",
    "category": "Συνεργάτης",
    "phone": "2810280280",
    "email": "",
    "description": "TASTE ACADEMY - Γραμματεία"
  },
  {
    "id": "contact_57",
    "name": "Χαλίλης - CLEAN HEAVEN",
    "category": "Συνεργάτης",
    "phone": "6943668562",
    "email": "",
    "description": "Καθαριότητα"
  },

  // === ΠΡΟΜΗΘΕΥΤΕΣ ===
  {
    "id": "contact_100",
    "name": "Βελιβασάκης ΑΕ",
    "category": "Προμηθευτής",
    "phone": "2810542810",
    "email": "office@velivasakis.com",
    "description": "Άλευρα, άχνη, κρέμα γάλακτος, τυριά"
  },
  {
    "id": "contact_101",
    "name": "Καλουδάκης Μηνάς",
    "category": "Προμηθευτής",
    "phone": "2810222705",
    "email": "orders@kaloudakis.com",
    "description": "Α' ύλες Ζαχαροπλαστικής / Είδη συσκευασίας"
  },
  {
    "id": "contact_102",
    "name": "Κορώνη",
    "category": "Προμηθευτής",
    "phone": "2810261410",
    "email": "info@koronisa.gr",
    "description": "Α' ύλες Ζαχαροπλαστικής / Είδη συσκευασίας"
  },
  {
    "id": "contact_103",
    "name": "Παρλαμάς Γιάννης",
    "category": "Προμηθευτής",
    "phone": "2810222421",
    "email": "parlamas@gmail.com",
    "description": "Α' ύλες Ζαχαροπλαστικής / Είδη συσκευασίας"
  },
  {
    "id": "contact_104",
    "name": "Πουράτος HELLAS",
    "category": "Προμηθευτής",
    "phone": "6936759820",
    "email": "kvafeiadis@puratos.com",
    "description": "Σοκολάτες"
  },
  {
    "id": "contact_105",
    "name": "Ανδρουλάκης Γιάννης",
    "category": "Προμηθευτής",
    "phone": "6948100135",
    "email": "androulakis@hotmail.com",
    "description": "Έλαια - Ηλιέλαια"
  },
  {
    "id": "contact_106",
    "name": "Αυγοδιατροφική ΣΠ FOODS",
    "category": "Προμηθευτής",
    "phone": "6947378738",
    "email": "papasxoi@gmail.com",
    "description": "Αυγό, παστεριωμένο κρόκο και ασπράδι"
  },
  {
    "id": "contact_107",
    "name": "Βιδάκης Γιάννης",
    "category": "Προμηθευτής",
    "phone": "6977559017",
    "email": "vidakis@vidakis.gr",
    "description": "Ξηροί καρποί"
  },
  {
    "id": "contact_108",
    "name": "Δασκαλάκης",
    "category": "Προμηθευτής",
    "phone": "2810333094",
    "email": "",
    "description": "Ψάρια, Θαλασσινά, κατεψυγμένα"
  },
  {
    "id": "contact_109",
    "name": "Ηλιάδης Νίκος",
    "category": "Προμηθευτής",
    "phone": "6937265582",
    "email": "sales@enopolio.gr",
    "description": "Κάβα - Ποτά"
  },
  {
    "id": "contact_110",
    "name": "Καλημέρα Φρούτα",
    "category": "Προμηθευτής",
    "phone": "6930818929",
    "email": "accounts@kalimerafrouta.gr",
    "description": "Φρούτα - Λαχανικά"
  },
  {
    "id": "contact_111",
    "name": "Κουρλετάκης Μανώλης",
    "category": "Προμηθευτής",
    "phone": "6932768313",
    "email": "",
    "description": "Έλαια, μαργαρίνη"
  },
  {
    "id": "contact_112",
    "name": "Μπελιμπασάκης Δημήτρης - Cerci",
    "category": "Προμηθευτής",
    "phone": "6978946962",
    "email": "",
    "description": "Α' ύλες κρέμα γάλακτος - Σοκολάτα"
  },
  {
    "id": "contact_113",
    "name": "Μύλοι Κρήτης",
    "category": "Προμηθευτής",
    "phone": "",
    "email": "ddp@mills.gr",
    "description": "Άλευρα"
  },
  {
    "id": "contact_114",
    "name": "Πανακάκης Μανώλης",
    "category": "Προμηθευτής",
    "phone": "6936903496",
    "email": "panakakis.emm@gmail.com",
    "description": "Α' ύλες Ζαχαροπλαστικής / Μαγειρικής, Μπαχαρικά, αλάτι"
  },
  {
    "id": "contact_115",
    "name": "Προκοπάκης Γιάννης",
    "category": "Προμηθευτής",
    "phone": "6947286614",
    "email": "",
    "description": "Τυροκομικά - Μυζήθρα"
  },
  {
    "id": "contact_116",
    "name": "Ρινακάκης",
    "category": "Προμηθευτής",
    "phone": "2810210426",
    "email": "",
    "description": "Φρούτα - Λαχανικά"
  },
  {
    "id": "contact_117",
    "name": "Σιδέρης Χαράλαμπος",
    "category": "Προμηθευτής",
    "phone": "6972096317",
    "email": "",
    "description": "Φύλλο κρούστας - Καταΐφι"
  },
  {
    "id": "contact_118",
    "name": "Σταθάκης",
    "category": "Προμηθευτής",
    "phone": "2810281343",
    "email": "info@stathakis-market.gr",
    "description": "Μπαχαρικά"
  },
  {
    "id": "contact_119",
    "name": "Σταυρακάκης Μύρων",
    "category": "Προμηθευτής",
    "phone": "6973552131",
    "email": "",
    "description": "Α' ύλες Ζαχαροπλαστικής"
  },
  {
    "id": "contact_120",
    "name": "Συλιγάρδος ΟΕ (Νίκος)",
    "category": "Προμηθευτής",
    "phone": "6973556169",
    "email": "",
    "description": "Φρέσκο αιγοπρόβιο γάλα, τυροκομικά, μυζήθρα"
  },
  {
    "id": "contact_121",
    "name": "Σαριδάκης Γιάννης",
    "category": "Προμηθευτής",
    "phone": "6998488488",
    "email": "info@sww.gr",
    "description": "Στολές εργασίας"
  },
  {
    "id": "contact_122",
    "name": "Αλιμπράντης Βαγγέλης",
    "category": "Προμηθευτής",
    "phone": "6944458051",
    "email": "",
    "description": "Ζαχαρόπαστα, είδη διακόσμησης"
  },
  {
    "id": "contact_123",
    "name": "Βιοερευνητικά Εργαστήρια Στεφανάκη Στέλλα",
    "category": "Προμηθευτής",
    "phone": "6948008997",
    "email": "desk@biolabcrete.gr",
    "description": "Ανάλυση νερού - Χημείο"
  },
  {
    "id": "contact_124",
    "name": "Ασπυρτάκης Γιώργος - Metrocal",
    "category": "Προμηθευτής",
    "phone": "6974067609",
    "email": "",
    "description": "Διακρίβωση οργάνων"
  }
];

// ==========================================
// COURSES BY SPECIALTY - Μαθήματα ανά Τμήμα
// ==========================================
export const COURSES_BY_SPECIALTY = {
  "spec_11": {
    "title": "Εσωτερική Αρχιτεκτονική Διακόσμηση και Σχεδιασμός Αντικειμένων",
    "semester1": [
      "Φυσικά Πετρώματα",
      "Παλαιότεροι Πολιτισμοί και Τέχνη",
      "Γραμμικό Σχέδιο",
      "Ελεύθερο και Βασικό Σχέδιο",
      "Χρώμα και Διακοσμητικές Εφαρμογές",
      "Πρακτική Εφαρμογή σε Χώρο Βιτρίνας"
    ],
    "semester2": [
      "Παράγωγα Υλικά",
      "Αναγέννηση και Νεότερη Τέχνη",
      "Μεθοδολογία Μελέτης Διακόσμησης",
      "Τρισδιάστατη Διακοσμητική Απεικόνιση",
      "Δισδιάστατη Ψηφιακή Σχεδίαση",
      "Δομικά Στοιχεία Εσωτερικών Χώρων",
      "Πρακτική Εφαρμογή σε Χώρους Κατοικίας"
    ],
    "semester3": [
      "Τεχνητά Υλικά",
      "Σύγχρονη και Ελληνική Τέχνη",
      "Κατασκευαστικό Σχέδιο Εσωτερικών Χώρων",
      "Διακοσμητικές Εφαρμογές Χρηστικών Αντικειμένων",
      "Πρακτική Εφαρμογή σε Χώρους Εργασίας"
    ]
  },
  "spec_13": {
    "title": "Τεχνικός Αρτοποιίας – Ζαχαροπλαστικής",
    "semester1": [
      "Ζαχαροπλαστική Τέχνη Ι",
      "Στοιχεία Μαγειρικής (Catering) Ι",
      "Υγιεινή και Ασφάλεια",
      "Τρόφογνωσία",
      "Αγγλικά Ι",
      "Πρακτική Εφαρμογή στην Ειδικότητα"
    ],
    "semester2": [
      "Ζαχαροπλαστική Τέχνη ΙΙ",
      "Βασικές Αρχές Διατροφής",
      "Διαχείριση Πρώτων Υλών Ι (F&B Management I)",
      "Αγγλικά ΙΙ",
      "Πρακτική Εφαρμογή στην Ειδικότητα"
    ],
    "semester3": [
      "Ζαχαροπλαστική Τέχνη ΙΙΙ",
      "Αρτοποιητική Τέχνη Ι",
      "Αγγλικά Ι",
      "Διαχείριση Πρώτων Υλών ΙΙ (F&B Management ΙΙ) - Κοστολόγηση",
      "Οργάνωση και Λειτουργία Αρτοζαχαροπλαστείου",
      "Πρακτική Εφαρμογή στην Ειδικότητα"
    ],
    "semester4": [
      "Ζαχαροπλαστική Τέχνη ΙV",
      "Αρτοποιητική Τέχνη ΙΙ",
      "Αγγλικά ΙΙ",
      "Βιώσιμη Ανάπτυξη στον Τουρισμό",
      "Πρακτική Εφαρμογή στην Ειδικότητα"
    ]
  },
  "spec_12": {
    "title": "Τεχνικός Μαγειρικής Τέχνης – Αρχιμάγειρας (Chef)",
    "semester1": [
      "Γαλλικά Ι",
      "Τέχνη Μαγειρικής Ι",
      "Στοιχεία Ζαχαροπλαστικής Ι",
      "Υγιεινή και Ασφάλεια",
      "Τρόφογνωσία",
      "Πρακτική Εφαρμογή Α΄"
    ],
    "semester2": [
      "Γαλλικά ΙΙ",
      "Τέχνη Μαγειρικής ΙΙ",
      "Στοιχεία Ζαχαροπλαστικής ΙΙ",
      "Εργαστήριο ΠΕ",
      "Βασικές Αρχές Διατροφής",
      "Food and Beverage Management (Έλεγχος Εμπορευμάτων)"
    ]
  },
  "spec_16": {
    "title": "Στέλεχος Υπηρεσιών Αερομεταφοράς",
    "semester1": [
      "Business English & Tourism Terminology",
      "Γαλλικά Α",
      "Τουρισμός και Αεροπορική Βιομηχανία Μεταφορών Α",
      "Ταξιδιωτική Γεωγραφία και Ταξιδιωτικοί Οδηγοί Α",
      "Αεροπορικό Δίκαιο και Διεθνείς Σχέσεις (IATA – UFTAA – ICAO – BSP) Α",
      "Αεροπορικοί Ναύλοι Α",
      "Ασφάλεια και Υγιεινή",
      "Εισαγωγή στην Αεροπορική Ορολογία στα Αγγλικά",
      "Εισαγωγή στην Αεροπορική Ορολογία στα Γαλλικά",
      "Εισαγωγή στα Συστήματα Κρατήσεων Αεροπορικών Εισιτηρίων"
    ],
    "semester2": [
      "Business English & Tourism Terminology Β",
      "Γαλλικά Β",
      "Τουρισμός και Αεροπορική Βιομηχανία Μεταφορών Β",
      "Ταξιδιωτική Γεωγραφία και Ταξιδιωτικοί Οδηγοί Β",
      "Αεροπορικό Δίκαιο και Διεθνείς Σχέσεις Β",
      "Αεροπορικοί Ναύλοι Β",
      "Εισαγωγή στην Κατασκευή Αεροπορικού Ναύλου και Τιμολόγηση"
    ]
  },
  "spec_7": {
    "title": "Στέλεχος Διοίκησης και Οικονομίας",
    "semester1": [
      "Αστικό Δίκαιο",
      "Εισαγωγή στα Οικονομικά Μαθηματικά",
      "Εισαγωγή στη Διοίκηση Επιχειρήσεων",
      "Πρακτική Εφαρμογή στην Ειδικότητα"
    ],
    "semester2": [
      "Εισαγωγή στην Στατιστική",
      "Εισαγωγή στη Διοίκηση Επιχειρήσεων ΙΙ",
      "Πρακτική Εφαρμογή στην Ειδικότητα"
    ]
  }
};

export const INITIAL_GRADES = [
  { id: 'grd_fake1', studentId: 'stud_11_1', courseTitle: 'Φυσικά Πετρώματα', progressGrade: '8', finalGrade: '9' },
  { id: 'grd_fake2', studentId: 'stud_11_2', courseTitle: 'Φυσικά Πετρώματα', progressGrade: '6', finalGrade: '7' },
  { id: 'grd_fake3', studentId: 'stud_11_3', courseTitle: 'Φυσικά Πετρώματα', progressGrade: '4', finalGrade: '5' },

  { id: 'grd_fake4', studentId: 'stud_11_1', courseTitle: 'Γραμμικό Σχέδιο', progressGrade: '9', finalGrade: '10' },
  { id: 'grd_fake5', studentId: 'stud_11_2', courseTitle: 'Γραμμικό Σχέδιο', progressGrade: '7', finalGrade: '8' },

  { id: 'grd_fake6', studentId: 'stud_13_1', courseTitle: 'Ζαχαροπλαστική Τέχνη Ι', progressGrade: '10', finalGrade: '10' },
  { id: 'grd_fake7', studentId: 'stud_13_2', courseTitle: 'Ζαχαροπλαστική Τέχνη Ι', progressGrade: '8', finalGrade: '9' },
  { id: 'grd_fake8', studentId: 'stud_13_3', courseTitle: 'Ζαχαροπλαστική Τέχνη Ι', progressGrade: '5', finalGrade: '4' },

  { id: 'grd_fake9', studentId: 'stud_12_1', courseTitle: 'Τέχνη Μαγειρικής Ι', progressGrade: '7', finalGrade: '8' },
  { id: 'grd_fake10', studentId: 'stud_12_2', courseTitle: 'Τέχνη Μαγειρικής Ι', progressGrade: '9', finalGrade: '9' }
];

export const INITIAL_SECTIONS = [
  { id: 'sec_1', name: 'Α1 - Εσωτερικής Αρχιτεκτονικής', specialtyId: 'spec_11', semester: 'semester1' },
  { id: 'sec_2', name: 'Γ1 - Αρτοποιίας Ζαχαροπλαστικής', specialtyId: 'spec_13', semester: 'semester3' }
];
